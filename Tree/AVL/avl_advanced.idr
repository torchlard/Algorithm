...
doInsert newKey newVal (Element (Node key val l r) (AVLNode invl invr b)) with (compare newKey key)
  doInsert newKey newVal (Element (Node key val l r) (AVLNode invl invr b)) 
    | EQ = Same (Element (Node newKey newVal l r) (AVLNode invl invr b))

  doInsert newKey newVal (Element (Node key val l r) (AVLNode invl invr b)) 
    | LT with (assert_total $ doInsert newKey newVal (Element l invl))
    -- Totality checker not clever enough to see that this is smaller
    doInsert newKey newVal (Element (Node key val l r) (AVLNode invl invr b))        
      | LT | (Same (Element l' invl'))
        = Same $ Element (Node key val l' r) (AVLNode invl' invr b)
    doInsert newKey newVal (Element (Node key val l r) (AVLNode invl invr LHeavy))   
      | LT | (Grew (Element l' invl'))
        = rotRight key val (Element l' invl') (Element r invr)
    doInsert newKey newVal (Element (Node key val l r) (AVLNode invl invr Balanced)) 
      | LT | (Grew (Element l' invl'))
        = Grew $ Element (Node key val l' r) (AVLNode invl' invr LHeavy)
    doInsert newKey newVal (Element (Node key val l r) (AVLNode invl invr RHeavy))   
      | LT | (Grew (Element l' invl'))
